...
public:
  ...
  bool globalExists(llvm::StringRef name) const { return globalAnnotations.find(name) != globalAnnotations.end(); }
  ...
  void insertFunction(StringOrAnnotation annotation, llvm::StringRef functionName) {
    functionAnnotations[functionName] = std::move(annotation);
  }
  void insertLocal(StringOrAnnotation annotation, llvm::StringRef varName, llvm::StringRef functionName) {
    localAnnotations[functionName][varName] = std::move(annotation);
  }

  void insertStruct(StructAnnotation strct, llvm::StringRef strName) { symbolTable[strName] = std::move(strct); }

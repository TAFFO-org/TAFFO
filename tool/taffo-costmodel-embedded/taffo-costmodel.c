...
const char* types_names[N_T] = {"FIX", "FLOAT", "DOUBLE"};

uint64_t op_times[N_OP][N_T];
uint64_t cast_times[N_OP][N_OP];

#define N_ITER (1024 * 512)
...
#define MEASURE_BIN_OP(type_id, type, op_id, op)         \
  do {                                                   \
...
#define MEASURE_CAST_OP(type_a_id, type_a, type_b_id, type_b, op, r_op) \
  do {                                                                  \

...
#define UnpackedDualSel(a, b) (static_cast<int>(a) + static_cast<int>(b) * 4)
#endif
  ...
  CONSTEXPR14 Unpacked normalized() const {
    if (fraction == 0) {
      return Unpacked(Type::Zero, false);
    } else {
  ...
  constexpr bool isInfinity() const { return type == Type::Infinity; }
  constexpr bool isRegular() const { return type == Type::Regular; }
  constexpr bool isNaN() const { return type == Type::NaN; }
  constexpr bool isZero() const { return type == Type::Zero; }
  constexpr bool isPositive() const { return !negativeSign; }

  static constexpr Unpacked infinity() { return Unpacked(Type::Infinity); }
  static constexpr Unpacked pinfinity() {
    return Unpacked(Type::Infinity, false);
  }
  static constexpr Unpacked ninfinity() {
    return Unpacked(Type::Infinity, true);
  }
  static constexpr Unpacked nan() { return Unpacked(Type::NaN); }
  static constexpr Unpacked one() { return Unpacked(0, 0, false); }
  static constexpr Unpacked zero() { return Unpacked(Type::Zero); }
  template <class Trait>
  ...
  constexpr bool operator==(const Unpacked &u) const {
    // nan != nan ALWAYS
    return type == Type::NaN || u.type == Type::NaN
               ? false
  ...
  constexpr bool operator!=(const Unpacked &u) const {
    // nan != nan ALWAYS
    return type == Type::NaN || u.type == Type::NaN ? true : (*this == u);
  }
        ...
        if (fraction == 0) {
          ...
          return Unpacked(
              ...
              nextintop<FT>::extradiv(
                  ((typename nextinttype<FT>::type)afrac) << FT_bits, bfrac),
              negativeSign);
  ...
  CONSTEXPR14 friend Unpacked operator+(Unpacked a, Unpacked b) {
    ...
    switch (UnpackedDualSel(a.type, b.type)) {
      ...
      case UnpackedDualSel(Type::Zero, Type::Zero):
      case UnpackedDualSel(Type::Infinity, Type::Zero):
      case UnpackedDualSel(Type::Infinity, Type::Regular):
        return a;
      case UnpackedDualSel(Type::Zero, Type::Regular):
      case UnpackedDualSel(Type::Zero, Type::Infinity):
      case UnpackedDualSel(Type::Regular, Type::Infinity):
        return b;
  ...
  /**
   * Soft Float division based on the 16 possible cases of the typed numbers
   * @param a dividend
   * @param b divisor
  ...
  CONSTEXPR14 friend Unpacked operator/(const Unpacked &a, const Unpacked &b) {
    if (a.isNaN() || b.isNaN()) {
      ...
      switch (UnpackedDualSel(a.type, b.type)) {
        ...
        case UnpackedDualSel(Type::Zero, Type::Zero):
        case UnpackedDualSel(Type::Infinity, Type::Infinity):
          return nan();
        case UnpackedDualSel(Type::Zero, Type::Infinity):
          return zero();
        case UnpackedDualSel(Type::Zero, Type::Regular):
        case UnpackedDualSel(Type::Infinity, Type::Zero):
          return a;
        case UnpackedDualSel(Type::Regular, Type::Zero):
          return Unpacked(Type::Infinity, a.negativeSign);
        default:
          return (a.negativeSign ^ b.negativeSign) ? ninfinity() : pinfinity();
      }
    }
  ...
  friend std::ostream &operator<<(std::ostream &ons, Unpacked const &o) {
    switch (o.type) {
      case Unpacked::Type::Regular:
        ...
        break;
      case Unpacked::Type::Infinity:
        ons << (o.negativeSign ? "up(-infinity)" : "up(+infinity)");
        break;
      case Unpacked::Type::NaN:
        ons << "up(nan)";
        break;
      case Unpacked::Type::Zero:
        ons << "up(0)";
        break;
    }
    return ons;
...
CONSTEXPR14 Unpacked<FT, ET> &Unpacked<FT, ET>::unpack_xfixed(
  ...
  if (nx != 0) {
    using UT = typename int_least_bits<sizeof(typename Trait::value_t) *
                                       8>::unsigned_type;
    type = Type::Regular;
    negativeSign = nx < 0;
    UT x = pcabs(nx);
...
CONSTEXPR14 Unpacked<FT, ET> &Unpacked<FT, ET>::unpack_xfloati(
  ...
  if (rawexp == ((1 << Trait::exponent_bits) - 1))  // AKA 128 for single
  {
    if (fraction == 0) {
      ...
      type = Type::NaN;  // don't care which
    }
  } else if (rawexp == 0) {
    // normalized
...
CONSTEXPR14 typename Trait::holder_t Unpacked<FT, ET>::pack_xfloati() const {
  ...
  largest_type<ET, typename int_least_bits<Trait::exponent_bits>::signed_type>
      fexp = exponent;
...
// minimal fixed type for casting
template <class ET, class FT, class FXT>
struct lightfixed {
  using UT = Unpacked<ET, FT>;
  using value_t = typename FXT::value_t;

  lightfixed(UT ut) : v(ut.template pack_xfixed<FXT>()) {}

  lightfixed() : v(0) {}

  lightfixed(value_t x) : v(x) {}

  operator UT() { return UT::template make_fixed<FXT>(v); }

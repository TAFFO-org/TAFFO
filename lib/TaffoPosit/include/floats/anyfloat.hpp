...
public:
    ...
    using UnpackedT=posit::Unpacked<holder_t,exponenttype>;
	...
	anyfloat_emu& operator-=(const anyfloat_emu &a) { anyfloat_emu r = *this-a; v = r.v; return *this; }
	anyfloat_emu& operator/=(const anyfloat_emu &a) { anyfloat_emu r = *this/a; v = r.v; return *this; }
	anyfloat_emu& operator=(const float &a) { anyfloat_emu r(a); v = r.v; return *this; }
	...
	//friend std::ostream & operator << (std::ostream &ons,const anyfloat_emu<expbits,fractionbits,value_t,holder_t,impl_t,with_denorm_>&b );

};

template <int expbits, int fractionbits, class value_t, class holder_t, class impl_t, bool with_denorm_ = true>
std::ostream & operator << (std::ostream &ons,const anyfloat_emu<expbits,fractionbits,value_t,holder_t,impl_t,with_denorm_>&b ) {
    ons << float(b);
    return ons;
}
...
using binary32_emu = anyfloat_emu<8,23,singlefloat,uint32_t,float>;

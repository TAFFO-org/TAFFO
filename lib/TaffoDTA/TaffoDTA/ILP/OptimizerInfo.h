...
public:
  ...
  OptimizerInfo(OptimizerInfoKind K)
  : Kind(K) {}
  ...
  virtual std::string toString() const { return "OptimizerInfo"; }

  virtual bool operator==(const OptimizerInfo& b) const { return Kind == b.Kind; }
...
struct OptimizerScalarInfo : public OptimizerInfo {
  ...
  OptimizerScalarInfo(std::string _variableName,
                      unsigned _minBits,
                      unsigned _maxBits,
                      unsigned _totalBits,
                      bool _isSigned,
                      taffo::Range _range,
                      std::string _overriddenEnob)
  ...
  std::shared_ptr<taffo::Range> getRange() { return range; }

  const std::string getBaseName() const { return *baseName.get(); }

  std::string getOverridedEnob() { return overridedEnob; }

  bool doesReferToConstant() const { return referToConstant; }

  void setReferToConstant(bool referToConstant) { this->referToConstant = referToConstant; }
  ...
  const std::string getFloatSelectedVariable() { return *baseName + "_float"; }

  const std::string getDoubleSelectedVariable() { return *baseName + "_double"; }

  const std::string getHalfSelectedVariable() { return *baseName + "_Half"; }

  const std::string getQuadSelectedVariable() { return *baseName + "_Quad"; }

  const std::string getFP80SelectedVariable() { return *baseName + "_FP80"; }

  const std::string getPPC128SelectedVariable() { return *baseName + "_PPC128"; }

  const std::string getBF16SelectedVariable() { return *baseName + "_BF16"; }

  const std::string getFractBitsVariable() { return *baseName + "_fixbits"; }
  ...
  void overrideEnob(std::string newEnob) { overridedEnob = newEnob; }
...
public:
  ...
  const std::shared_ptr<OptimizerInfo>& getOptInfo() const { return optInfo; }

...
BiMap<std::string, Value*> MetadataManager::getIdValueMapping(Module& m) {
  ...
  // Global values:
  if (NamedMDNode* globalMappingMd = m.getNamedMetadata(TAFFO_GLOBAL_ID_LIST_METADATA)) {
    for (MDNode* entryMd : globalMappingMd->operands()) {
      std::string id = cast<MDString>(entryMd->getOperand(0))->getString().str();
      auto* valueMd = dyn_cast_or_null<ValueAsMetadata>(entryMd->getOperand(1));
      if (!valueMd) {
  ...
  for (Function& f : m) {
    ...
    if (argsMd) {
      for (const MDOperand& mdOperand : argsMd->operands()) {
        auto* entry = cast<MDNode>(mdOperand);
        unsigned argIndex =
          cast<ConstantInt>(cast<ConstantAsMetadata>(entry->getOperand(0))->getValue())->getZExtValue();
...
std::unordered_map<StructType*, StructPaddingInfo> MetadataManager::getStructPaddingInfo(Module& m) {
...
void MetadataManager::getCudaKernels(Module& m, SmallVectorImpl<Function*> kernels) {
  ...
  for (MDNode* entry : cudaMd->operands()) {
    if (entry->getNumOperands() < 2)
      continue;
    auto* kindId = dyn_cast<MDString>(entry->getOperand(1));

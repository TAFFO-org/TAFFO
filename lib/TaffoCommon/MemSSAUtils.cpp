...
SmallVectorImpl<Value*>& MemSSAUtils::getDefiningValues(Instruction* i) {
  Visited.clear();
  Res.clear();
...
void MemSSAUtils::findMemPhiValue(Instruction* i, MemoryPhi* mphi) {
...
void MemSSAUtils::findClobberingValues(Instruction* i, MemoryAccess* ma) {
  if (ma == nullptr)
    return;

...
Value* ConversionPass::convertInstruction(Module& m, Instruction* val, std::shared_ptr<FixedPointType>& fixpt) {
  ...
  if (res) {
    if (!isa<Instruction>(res))
      LLVM_DEBUG(log() << "Conversion produced something that is not an instruction from an instruction...\n");
    return res;
  }
  else {
...
Value* ConversionPass::convertAlloca(AllocaInst* alloca, const std::shared_ptr<FixedPointType>& fixpt) {
  ...
  AllocaInst* newinst = new AllocaInst(newAllocatedType, alloca->getType()->getPointerAddressSpace(), as, align);
...
Value* ConversionPass::convertLoad(LoadInst* load, std::shared_ptr<FixedPointType>& fixpt, Module& m) {
  ...
  if (isConvertedFixedPoint(newptr)) {
    ...
    Type* PELType = getFullyUnwrappedType(newptr);
    Align align;
  ...
  if (isFloatingPointToConvert(val)) {
    ...
    if (isConvertedFixedPoint(newPtrOperand)) {
      ...
      if (newPointedOperandType->isPointerType()) {
        ...
        if (*getFixpType(newval) != *valtype)
          LLVM_DEBUG(log() << "unsolvable fixp type mismatch between store dest and src!\n");
      }
      else {
    ...
    if (isConvertedFixedPoint(newPtrOperand)) {
      ...
      if (newPointedOperandType->isIntegerType() || !newPointedOperandType->isPointerType()) {
        ...
        LLVM_DEBUG(log() << "[Store] HACK: bitcasting operands of wrong type to new type\n");
...
Value* ConversionPass::convertGep(GetElementPtrInst* gep, std::shared_ptr<FixedPointType>& fixpt) {
  ...
  std::shared_ptr<TransparentType> type =
    TaffoInfo::getInstance().getOrCreateTransparentType(*gep->getPointerOperand());
...
Value* ConversionPass::convertExtractValue(ExtractValueInst* exv, std::shared_ptr<FixedPointType>& fixpt) {
  ...
  if (!newval)
    return nullptr;
  std::shared_ptr<FixedPointType> baset = getFixpType(newval)->unwrapIndexList(
    TaffoInfo::getInstance().getOrCreateTransparentType(*oldval), exv->getIndices());
  ...
  if (!baset->isInvalid() && newi->getType()->isIntegerTy())
    return genConvertFixedToFixed(
      newi, std::static_ptr_cast<FixedPointScalarType>(baset), std::static_ptr_cast<FixedPointScalarType>(fixpt));
  fixpt = baset;
  return newi;
}
...
Value* ConversionPass::convertPhi(PHINode* phi, std::shared_ptr<FixedPointType>& fixpt) {
  ...
  PHINode* newphi =
    PHINode::Create(std::static_ptr_cast<FixedPointScalarType>(fixpt)->scalarToLLVMType(phi->getContext()),
...
Value* ConversionPass::convertCall(CallBase* call, std::shared_ptr<FixedPointType>& fixpt) {
  ...
  if (!newF) {
    LLVM_DEBUG(log() << "[Info] no function clone for instruction" << *(call) << ", engaging fallback\n");
    return Unsupported;
  }
  ...
  while (call_arg != call->arg_end()) {
    ...
    if (!thisArgument) {
      LLVM_DEBUG(log() << "CALL: match of argument " << i << " (" << *f_arg << ") failed\n");
      return Unsupported;
    }
...
Value* ConversionPass::convertUnaryOp(Instruction* inst, const std::shared_ptr<FixedPointType>& fixpt) {
  if (!inst->getType()->isFloatingPointTy() || getConversionInfo(inst)->isConversionDisabled)
    return Unsupported;
  ...
  if (opc == Instruction::FNeg) {
    ...
    Value* val1 = translateOrMatchOperandAndType(inst->getOperand(0), fixpt, inst);
    if (!val1)
      return nullptr;
...
Value* ConversionPass::convertBinOp(Instruction* inst, const std::shared_ptr<FixedPointScalarType>& dstType) {
  ...
  if (opc == Instruction::FAdd || opc == Instruction::FSub || opc == Instruction::FRem) {
    LLVM_DEBUG(log() << inst->getOperand(0) << "\n";);
    LLVM_DEBUG(log() << inst->getOperand(0) << "\n";);
    Value* val1 = translateOrMatchOperandAndType(inst->getOperand(0), dstType, inst);
    Value* val2 = translateOrMatchOperandAndType(inst->getOperand(1), dstType, inst);
    if (!val1 || !val2)
      return nullptr;
    IRBuilder<NoFolder> builder(inst);
    Value* fixop;
    ...
    if (dstType->isFixedPoint()) {
      Value* val1 = translateOrMatchOperand(inst->getOperand(0), intype1, inst, TypeMatchPolicy::RangeOverHintMaxInt);
      Value* val2 = translateOrMatchOperand(inst->getOperand(1), intype2, inst, TypeMatchPolicy::RangeOverHintMaxInt);
      if (!val1 || !val2)
        return nullptr;
      ...
      Type* dbfxt = intermType->scalarToLLVMType(inst->getContext());
      IRBuilder<NoFolder> builder(inst);
      Value* intermResult = nullptr;
      Value* ext1 = nullptr;
      Value* ext2 = nullptr;
      if (dbfxt->getScalarSizeInBits() > MaxTotalBitsConv) {
        dbfxt = dstType->scalarToLLVMType(inst->getContext());

        ext1 = val1;
        ext2 = val2;

        auto make_to_same_size = [this, inst, &builder](std::shared_ptr<FixedPointScalarType>& from,
                                                        std::shared_ptr<FixedPointScalarType>& to,
                                                        Value*& ext,
                                                        Value* val) {
          auto llvmfrom = from->scalarToLLVMType(inst->getContext());
          auto llvmto = to->scalarToLLVMType(inst->getContext());
        ...
        auto target_frac = dstType->getFractionalBits();
        int shift_right1 = 0;
        int shift_right2 = 0;
        ...
        copyValueInfo(intermResult, inst);
        updateFPTypeMetadata(
        ...
        return genConvertFixedToFixed(intermResult, intermType, dstType, inst);
      }
      else {
      ...
      Value* val1 = translateOrMatchOperand(inst->getOperand(0), intype1, inst, TypeMatchPolicy::ForceHint);
      Value* val2 = translateOrMatchOperand(inst->getOperand(1), intype2, inst, TypeMatchPolicy::ForceHint);
      if (!val1 || !val2)
        return nullptr;
      IRBuilder<NoFolder> builder(inst);
    ...
    std::shared_ptr<FixedPointType> intype2 = dstType->clone();
    if (dstType->isFixedPoint()) {
      Value* val1 = translateOrMatchOperand(inst->getOperand(0), intype1, inst, TypeMatchPolicy::RangeOverHintMaxFrac);
      Value* val2 = translateOrMatchOperand(inst->getOperand(1), intype2, inst, TypeMatchPolicy::RangeOverHintMaxInt);
      if (!val1 || !val2)
        return nullptr;
      std::shared_ptr<FixedPointScalarType> scalarIntype1 = std::static_ptr_cast<FixedPointScalarType>(intype1);
      std::shared_ptr<FixedPointScalarType> scalarIntype2 = std::static_ptr_cast<FixedPointScalarType>(intype2);
      LLVM_DEBUG(log() << "fdiv val1 = " << *val1 << " type = " << *scalarIntype1 << "\n");
      ...
      if (Size > MaxTotalBitsConv) {
        ...
        Size = dstType->getBits();
        unsigned target = dstType->getFractionalBits();
      ...
      Value* ext1 = genConvertFixedToFixed(val1, scalarIntype1, ext1type, inst);
      ...
      Value* ext2 = genConvertFixedToFixed(val2, scalarIntype2, ext2type, inst);

      /* Generate division */
      std::shared_ptr<FixedPointScalarType> intermType =
        std::make_shared<FixedPointScalarType>(SignedRes, Size, Ext1Exp - Ext2Exp);
      IRBuilder<NoFolder> builder(inst);
      Value* intermResult = dstType->isSigned() ? builder.CreateSDiv(ext1, ext2) : builder.CreateUDiv(ext1, ext2);

      LLVM_DEBUG(log() << "fdiv ext1 = " << *ext1 << " type = " << *ext1type << "\n");
      LLVM_DEBUG(log() << "fdiv ext2 = " << *ext2 << " type = " << *ext2type << "\n");
      LLVM_DEBUG(log() << "fdiv fixop = " << *intermResult << "type = " << *intermType << "\n");
      ...
      updateConstTypeMetadata(intermResult, 1U, ext2type);
      return genConvertFixedToFixed(intermResult, intermType, dstType, inst);
    }
    else if (dstType->isFloatingPoint()) {
      Value* val1 = translateOrMatchOperand(inst->getOperand(0), intype1, inst, TypeMatchPolicy::ForceHint);
...
Value* ConversionPass::convertCmp(FCmpInst* fcmp) {
  ...
  if (!isOneFloat) {
    ...
    int intpart2 = t2->getBits() - t2->getFractionalBits() + (mixedsign ? t2->isSigned() : 0);
    ...
    if (pr == CmpInst::FCMP_OEQ) {
      ...
      return builder.CreateICmpEQ(ConstantInt::get(Type::getInt32Ty(fcmp->getContext()), 0),
                                  ConstantInt::get(Type::getInt32Ty(fcmp->getContext()), 0));
    }
    else if (pr == CmpInst::FCMP_FALSE) {
      return builder.CreateICmpNE(ConstantInt::get(Type::getInt32Ty(fcmp->getContext()), 0),
                                  ConstantInt::get(Type::getInt32Ty(fcmp->getContext()), 0));
    }
    if (swapped)
    ...
    if (t1->isFloatingPoint() && t2->isFloatingPoint()) {
      ...
      else if (t1->scalarToLLVMType(fcmp->getContext())->getPrimitiveSizeInBits()
               < t2->scalarToLLVMType(fcmp->getContext())->getPrimitiveSizeInBits()) {
        // t2 is "more precise"
        cmptype = t2;
      }
...
Value* ConversionPass::convertCast(CastInst* cast, const std::shared_ptr<FixedPointType>& fixpt) {
  ...
  if (operand->getType()->isFloatingPointTy()) {
    ...
    if (cast->getOpcode() == Instruction::SIToFP) {
      return genConvertFixedToFixed(val,
                                    std::make_shared<FixedPointScalarType>(val->getType(), true),
                                    std::static_ptr_cast<FixedPointScalarType>(fixpt),
                                    cast);
    }
    else if (cast->getOpcode() == Instruction::UIToFP) {
      return genConvertFixedToFixed(val,
                                    std::make_shared<FixedPointScalarType>(val->getType(), false),
                                    std::static_ptr_cast<FixedPointScalarType>(fixpt),
                                    cast);
    }
  }
...
Value* ConversionPass::fallback(Instruction* unsupported, std::shared_ptr<FixedPointType>& fixpt) {
  auto& taffoInfo = TaffoInfo::getInstance();
  Value* fallval;
  Value* fixval;
  ...
  LLVM_DEBUG(log() << "[Fallback] attempt to wrap not supported operation:\n"
                   << *unsupported << "\n");
  FallbackCount++;
  for (int i = 0, n = unsupported->getNumOperands(); i < n; i++) {
    fallval = unsupported->getOperand(i);
    if (hasConversionInfo(fallval)) {
      LLVM_DEBUG(log() << "  Substituted operand number : " << i + 1 << " of " << n << "\n");
      fixval = fallbackMatchValue(fallval, taffoInfo.getTransparentType(*fallval), unsupported);
  ...
  Instruction* tmp;
  if (getConversionInfo(unsupported)->isConversionDisabled == false && !unsupported->isTerminator()) {
    tmp = unsupported->clone();
    if (!tmp->getType()->isVoidTy())
      tmp->setName(unsupported->getName() + ".flt");
    tmp->insertAfter(unsupported);
  }
  else {
    tmp = unsupported;
  }
  ...
  LLVM_DEBUG(log() << "  mutated operands to:\n"
                   << *tmp << "\n");
  if (tmp->getType()->isFloatingPointTy() && getConversionInfo(unsupported)->isConversionDisabled == false) {
    Value* fallbackv =

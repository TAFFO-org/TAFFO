...
std::shared_ptr<CodeAnalyzer> VRAnalyzer::clone() {
...
void VRAnalyzer::setPathLocalInfo(std::shared_ptr<CodeAnalyzer> SuccAnalyzer,
                                  Instruction *TermInstr, unsigned SuccIdx)
{
...
void VRAnalyzer::prepareForCall(Instruction *I,
...
void VRAnalyzer::returnFromCall(Instruction *I,
                                std::shared_ptr<AnalysisStore> FunctionStore)
{
  CallBase *CB = cast<CallBase>(I);
  assert(!CB->isIndirectCall());
...
void VRAnalyzer::handleLoadInstr(Instruction *I)
{
  ...
  if (std::shared_ptr<ScalarInfo> Scalar =
          std::dynamic_ptr_cast_or_null<ScalarInfo>(Loaded)) {
    ...
    for (Value *dval : def_vals) {
      if (dval &&
          load_ty->canLosslesslyBitCastTo(getUnwrappedType(dval)))
...
void VRAnalyzer::handleGEPInstr(const Instruction *I) {
  ...
  if (!extractGEPOffset(gepInst->getSourceElementType(),
                        iterator_range(gepInst->idx_begin(), gepInst->idx_end()),
                        Offset)) {
    return;
  }
...
void VRAnalyzer::handleCmpInstr(const Instruction *cmp)
{
  ...
  for (unsigned index = 0; index < cmp_i->getNumOperands(); index++) {
    ...
    if (std::shared_ptr<ScalarInfo> op_range =
            std::dynamic_ptr_cast_or_null<ScalarInfo>(getNode(op))) {
...
void VRAnalyzer::handlePhiNode(const Instruction *phi)
{
  ...
  for (unsigned index = 0U; index < phi_n->getNumIncomingValues(); index++) {
    ...
    if (!op_node)
      continue;
    if (std::shared_ptr<ValueInfoWithRange> op_range =
            std::dynamic_ptr_cast<ScalarInfo>(op_node)) {
...
void VRAnalyzer::handleSelect(const Instruction *i)
{
  ...
  std::shared_ptr<ValueInfoWithRange> res = getUnionRange(fetchRangeNode(sel->getFalseValue()),

...
/* DCE code. Must scan the entire live-out data.
   Can be used also to check the correctness of the output. */
static void print_array2(int m, DATA_TYPE POLYBENCH_1D(symmat, M, m))
...
void correlationCuda(int m,
  ...
  checkCudaErrors(
    cuLaunchKernel(kernels[0], grid1.x, grid1.y, grid1.z, block1.x, block1.y, block1.z, 0, NULL, args1, NULL));
  checkCudaErrors(cuCtxSynchronize());

  void* args2[7] = {&m, &n, &mean_gpu, &stddev_gpu, &data_gpu, &lfloat_n, &leps};
  checkCudaErrors(
    cuLaunchKernel(kernels[1], grid2.x, grid2.y, grid2.z, block2.x, block2.y, block2.z, 0, NULL, args2, NULL));
  checkCudaErrors(cuCtxSynchronize());
  ...
  checkCudaErrors(
    cuLaunchKernel(kernels[2], grid3.x, grid3.y, grid3.z, block3.x, block3.y, block3.z, 0, NULL, args3, NULL));
  checkCudaErrors(cuCtxSynchronize());
  ...
  checkCudaErrors(
    cuLaunchKernel(kernels[3], grid4.x, grid4.y, grid4.z, block4.x, block4.y, block4.z, 0, NULL, args4, NULL));
  checkCudaErrors(cuCtxSynchronize());
  ...
  cudaMemcpy(&(symmat_gpu[(M-1)*M + (M-1)]), &valueAtSymmatIndexMTimesMPlus1PlusMPoint, sizeof(DATA_TYPE),

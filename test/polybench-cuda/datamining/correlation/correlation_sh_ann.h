...
#define ANN_MEAN __attribute__((annotate("scalar(range(-2, 2) final) target('mean') bufferid('mean')")))
#define ANN_STD __attribute__((annotate("scalar(range(-2,2) final) target('std') bufferid('std')")))
#define ANN_SYMMAT __attribute__((annotate("scalar(range(-1,1) final) target('symmat') bufferid('symmat')")))

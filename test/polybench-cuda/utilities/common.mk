...
${EXECUTABLE}:
	${CLANGX} -O3 -DPOLYBENCH_STACK_ARRAYS --cuda-host-only -L/usr/local/cuda/lib64 -I/usr/local/cuda/include  -Wl,-rpath="../../../../toolchains/lib64"      \
...
${EXECUTABLE_TAFFO}: ${PTXFILE_TAFFO}
	...
	${CLANG} -O3 --cuda-host-only -L/usr/local/cuda/lib64 -lstdc++ -lm -Wl,-rpath="../../../../toolchains/lib64"       \
    -lcudart_static -ldl -lrt -pthread ${CFLAGS} ${EXECUTABLE_TAFFO}.s -o ${EXECUTABLE_TAFFO} -lcuda

${PTXFILE}:
	${CLANG} ${CUFILE} -I${PATH_TO_UTILS} --cuda-device-only -S -o ${PTXFILE}

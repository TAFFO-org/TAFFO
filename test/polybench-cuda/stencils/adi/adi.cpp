  ...
  for (int t = 0; t < _PB_TSTEPS; t++) {
    ...
    for (int i1 = 0; i1 < _PB_N; i1++) {
      for (int i2 = 0; i2 < _PB_N - 2; i2++) {
        X[i1][(N - i2 - 2)] =
          (X[i1][(N - 2 - i2)] - X[i1][(N - 2 - i2 - 1)] * A[i1][(N - i2 - 3)]) / B[i1][(N - 3 - i2)];
      }
    }
...
void compareResults(int n,
                    DATA_TYPE POLYBENCH_2D(B_cpu, N, N, n, n),
...
void adiCuda(int tsteps,
  ...
  for (int t = 0; t < _PB_TSTEPS; t++) {
    ...
    checkCudaErrors(
      cuLaunchKernel(kernels[1], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args1, NULL));
    checkCudaErrors(cuCtxSynchronize());

    checkCudaErrors(
      cuLaunchKernel(kernels[2], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args1, NULL));
    checkCudaErrors(cuCtxSynchronize());

    for (int i1 = 1; i1 < _PB_N; i1++) {
      void* args2[5] = {&n, &A_gpu, &B_gpu, &X_gpu, &i1};
      checkCudaErrors(
        cuLaunchKernel(kernels[3], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args2, NULL));
      checkCudaErrors(cuCtxSynchronize());
    }

    checkCudaErrors(
      cuLaunchKernel(kernels[4], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args1, NULL));
    checkCudaErrors(cuCtxSynchronize());

    for (int i1 = 0; i1 < _PB_N - 2; i1++) {
      void* args2[5] = {&n, &A_gpu, &B_gpu, &X_gpu, &i1};
      checkCudaErrors(
        cuLaunchKernel(kernels[5], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args2, NULL));
      checkCudaErrors(cuCtxSynchronize());
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  // compareResults(n, POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(B_outputFromGpu), POLYBENCH_ARRAY(X),
  // POLYBENCH_ARRAY(X_outputFromGpu));

...
void gramschmidtCuda(int ni, int nj, DATA_TYPE POLYBENCH_2D(A,NI,NJ,ni,nj), DATA_TYPE POLYBENCH_2D(R,NJ,NJ,nj,nj), DATA_TYPE POLYBENCH_2D(Q,NI,NJ,ni,nj),
					 DATA_TYPE POLYBENCH_2D(A_outputFromGpu,NI,NJ,ni,nj), DATA_TYPE POLYBENCH_2D(R_outputFromGpu,NJ,NJ,ni,nj), DATA_TYPE POLYBENCH_2D(Q_outputFromGpu,NI,NJ,ni,nj))
{
	...
	for (k = 0; k < _PB_NJ; k++)
	{
		...
		checkCudaErrors(cuLaunchKernel(
        	kernels[2], grid3.x, grid3.y, grid3.z, block.x, block.y, block.z,
        	0, NULL, args3, NULL));
		checkCudaErrors(cuCtxSynchronize());
	}
...
int main(int argc, char *argv[])
{
	...
	gramschmidtCuda(ni, nj, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(R), POLYBENCH_ARRAY(Q), POLYBENCH_ARRAY(A_outputFromGpu), POLYBENCH_ARRAY(R_outputFromGpu), POLYBENCH_ARRAY(Q_outputFromGpu));

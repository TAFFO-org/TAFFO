    ...
    for (iz = 0; iz < _PB_CZ; iz++) {
      for (iy = 0; iy < _PB_CYM; iy++) {
        for (ix = 0; ix < _PB_CXM; ix++) {
          ...
          Hz[iz][iy][ix] = (cxmh[ix] / cxph[ix]) * Hz[iz][iy][ix] + (mui * czp[iz] / cxph[ix]) * tmp[iz][iy]
        ...
        for (ix = 0; ix < _PB_CXM; ix++) {
          ...
          Hz[iz][_PB_CYM][ix] = (cxmh[ix] / cxph[ix]) * Hz[iz][_PB_CYM][ix] + (mui * czp[iz] / cxph[ix]) * tmp[iz][iy]
...
int main(int argc, char** argv) {
  ...
  polybench_prevent_dce(
    print_array(cz, cxm, cym, POLYBENCH_ARRAY(Bza), POLYBENCH_ARRAY(Ex), POLYBENCH_ARRAY(Ey), POLYBENCH_ARRAY(Hz)));

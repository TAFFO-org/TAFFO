...
static void kernel_dynprog(int tsteps,
                           int length,
                           DATA_TYPE POLYBENCH_2D(c, LENGTH, LENGTH, length, length),
                           DATA_TYPE POLYBENCH_2D(W, LENGTH, LENGTH, length, length),
...
#pragma omp for private(j)
      for (i = 0; i <= _PB_LENGTH - 1; i++) for (j = 0; j <= _PB_LENGTH - 1; j++) c[i][j] = 0;
...
int main(int argc, char** argv) {
  ...
  /* Run kernel. */
  kernel_dynprog(tsteps, length, POLYBENCH_ARRAY(c), POLYBENCH_ARRAY(W), POLYBENCH_ARRAY(sum_c), &out);

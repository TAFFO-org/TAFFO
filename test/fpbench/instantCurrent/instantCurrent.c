...
float ex0(float t, float resistance, float frequency, float inductance, float maxVoltage) {
  ...
  float __attribute__((annotate("scalar(range(-1, 2503))"))) denom =
...
int main() {
  ...
  float __attribute__((annotate("target('main') scalar(range(-2, 300))"))) t[len];
  ...
  float __attribute__((annotate("scalar(range(-2, 2))"))) inductance[len];

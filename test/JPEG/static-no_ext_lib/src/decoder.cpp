...
#define  kBlockSide 8
#define  kBlockSize (kBlockSide * kBlockSide)
#define  kMaxCodeLength 16
...
namespace {
    ...
    std::vector<int> layoutTables(double **tables, int nRows, int nCols,
                                  int V, int H, int Vmax, int Hmax, int tableIteratorMaxValue) {
        ...
        for (int tableIterator = 0; tableIterator < tableIteratorMaxValue; tableIterator++) {
            for (int v = 0; v < V; ++v) {
                for (int h = 0; h < H; ++h) {
                            ...
                            for (int i = 0; i < rowDecimation; ++i) {
                                for (int j = 0; j < colDecimation; ++j) {
                                    ...
                                    if (r < nRows && c < nCols) {
                                        ...
                                        //pixels[r * nCols + c] = (int)((tables[tableIterator])[tableCounter] );

                                        //double temp = (tables[tableIterator])[tableCounter];
                                        //printf("%f    ", temp);
                                        pixels[r * nCols + c] = (int) ((tables[tableIterator])[tableCounter] + 0.55555);
                                        //printf("%d\n", pixels[r * nCols + c]);
                                        //
                                        // printf("%f\n", temp);
...
Image decode(std::istream &stream) {
    ...
    //from there all the values of the image are fixed, so we can do further dynamic optimization
    const SOF0Data &sof0 = jpeg.sof0;

    int nComponents = sof0.nComponents;

    double **colorTables[nComponents] __attribute((annotate("target('colorTables') scalar(range(-1024, 1024))")));

    clock_t t;
    t = clock();

    int tableIteratorSize[nComponents];
    ...
    for (int c = 0; c < nComponents; ++c) {
        ...
        tableIteratorSize[c] = jpeg.componentTables[c].size();

        colorTables[c] = static_cast<double **>(malloc(sizeof(double *) * jpeg.componentTables[c].size()));
        ...
        for (IntTable &table : jpeg.componentTables[c]) {
            ...
            colorTables[c][step] = (double *) (malloc(sizeof(double) * kBlockSize));

            idct2(table.data(), colorTables[c][step]);
    ...
    for (int c = 0; c < nComponents; ++c) {
        channels[c] = layoutTables(colorTables[c], nRows, nCols,
                                   sof0.V[c], sof0.H[c], sof0.Vmax, sof0.Hmax, tableIteratorSize[c]);
    }

    /*Free allocated vectors*/
    for (int c = 0; c < nComponents; ++c) {
        for (int j = 0; j < tableIteratorSize[c]; j++) {
    ...
    double r __attribute((annotate("target('r') scalar(range(-1024, 1024))")));
    double g __attribute((annotate("target('g') scalar(range(-1024, 1024))")));
    ...
    if ((dir = opendir(path)) != nullptr) {
        ...
        while ((ent = readdir(dir)) != nullptr) {
            if (ent->d_type == DT_REG) {
                ...
                //write_bmp_file((std::string(ent->d_name)+std::string("test.bmp")).c_str(), decoded.nCols, decoded.nRows,  24, decoded.pixels);

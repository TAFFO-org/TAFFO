...
int BENCH_MAIN(int argc, char** argv)
{
  ...
  POLYBENCH_2D_ARRAY_DECL(B,DATA_TYPE __attribute__((annotate("scalar(range(" PB_XSTR(VAR_B_MIN) "," PB_XSTR(VAR_B_MAX) "))"))),N,M,n,m);
  ...
  for (int benchmark_i = 0; benchmark_i < BENCH_NUM_ITERATIONS; benchmark_i++) {
    ...
    randomize_2d(N, M, B, POLYBENCH_RANDOMIZE_RANGE);

...
#ifndef __TAFFO__
  fp = fopen("3DConvolution.ptx", "r");
#else
  fp = fopen("3DConvolution.taffo.ptx", "r");
#endif
  if (!fp) {
...
void cl_mem_init(DATA_TYPE POLYBENCH_3D(A, NI, NJ, NK, ni, nj, nk), DATA_TYPE POLYBENCH_3D(B, NI, NJ, NK, ni, nj, nk)) {
  ...
  errcode =
    clEnqueueWriteBuffer(clCommandQue, a_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * NI * NJ * NK, A, 0, NULL, NULL);
  errcode =
    clEnqueueWriteBuffer(clCommandQue, b_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * NI * NJ * NK, B, 0, NULL, NULL);
  if (errcode != CL_SUCCESS)

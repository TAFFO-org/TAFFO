...
#ifndef __TAFFO__
  fp = fopen("gramschmidt.ptx", "r");
#else
  fp = fopen("gramschmidt.taffo.ptx", "r");
#endif
  if (!fp) {
...
void cl_launch_kernel(int ni, int nj) {
  ...
  for (k = 0; k < NJ; k++) {
    ...
    errcode =
      clEnqueueNDRangeKernel(clCommandQue, clKernel1, 1, NULL, globalWorkSizeKernel1, localWorkSize, 0, NULL, NULL);
    if (errcode != CL_SUCCESS)
    ...
    // Execute the OpenCL kernel
    errcode =
      clEnqueueNDRangeKernel(clCommandQue, clKernel2, 1, NULL, globalWorkSizeKernel2, localWorkSize, 0, NULL, NULL);
    if (errcode != CL_SUCCESS)
      printf("Error in launching kernel2\n");
    clEnqueueBarrier(clCommandQue);

    globalWorkSizeKernel3[0] =
      (size_t) ceil(((float) NJ - (float) (k + 1)) / ((float) DIM_LOCAL_WORK_GROUP_X)) * DIM_LOCAL_WORK_GROUP_X;
    if (globalWorkSizeKernel3[0] > 1) {
      ...
      // Execute the OpenCL kernel
      errcode =
        clEnqueueNDRangeKernel(clCommandQue, clKernel3, 1, NULL, globalWorkSizeKernel3, localWorkSize, 0, NULL, NULL);
      if (errcode != CL_SUCCESS)
...
// Gram-Schmidt decomposition of A into Q and R s.t. A = Q*R;
...
int main(int argc, char* argv[]) {
  ...
  errcode = clEnqueueReadBuffer(
    clCommandQue, r_mem_obj, CL_TRUE, 0, NJ * NJ * sizeof(DATA_TYPE), POLYBENCH_ARRAY(R_outputFromGpu), 0, NULL, NULL);
  if (errcode != CL_SUCCESS)
    printf("Error in reading GPU mem\n");
  errcode = clEnqueueReadBuffer(
    clCommandQue, q_mem_obj, CL_TRUE, 0, NI * NJ * sizeof(DATA_TYPE), POLYBENCH_ARRAY(Q_outputFromGpu), 0, NULL, NULL);
  if (errcode != CL_SUCCESS)
    printf("Error in reading GPU mem\n");
  ...
  compareResults(ni, nj, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(Q_outputFromGpu));

#else  // prevent dead code elimination

  polybench_prevent_dce(print_array(ni, nj, POLYBENCH_ARRAY(Q_outputFromGpu)));

#endif // RUN_ON_CPU
  // for (int i=0; i<nj; i++) {
  //   for (int j=0; j<nj; j++) {
  //     fprintf(stderr, "%e ", R_gpu[i][j]);
  //   }
  //   fprintf(stderr, "\n");
  // }

...
void cl_mem_init(DATA_TYPE POLYBENCH_2D(A, NX, NY, nx, ny),
                 DATA_TYPE POLYBENCH_1D(r, NX, nx),
                 DATA_TYPE POLYBENCH_1D(s, NX, nx),
                 DATA_TYPE POLYBENCH_1D(p, NX, nx),
                 DATA_TYPE POLYBENCH_1D(q, NX, nx)) {
...
int main(int argc, char* argv[]) {
  ...
  errcode = clEnqueueReadBuffer(
    clCommandQue, s_mem_obj, CL_TRUE, 0, NY * sizeof(DATA_TYPE), POLYBENCH_ARRAY(s_outputFromGpu), 0, NULL, NULL);
  errcode = clEnqueueReadBuffer(
    clCommandQue, q_mem_obj, CL_TRUE, 0, NX * sizeof(DATA_TYPE), POLYBENCH_ARRAY(q_outputFromGpu), 0, NULL, NULL);
  if (errcode != CL_SUCCESS)
  ...
  compareResults(
    nx, ny, POLYBENCH_ARRAY(s), POLYBENCH_ARRAY(s_outputFromGpu), POLYBENCH_ARRAY(q), POLYBENCH_ARRAY(q_outputFromGpu));

...
#ifndef __TAFFO__
  fp = fopen("fdtd2d.ptx", "r");
#else
  fp = fopen("fdtd2d.taffo.ptx", "r");
#endif
  if (!fp) {
...
void cl_mem_init(DATA_TYPE POLYBENCH_1D(_fict_, TMAX, tmax),
                 DATA_TYPE POLYBENCH_2D(ex, NX, NY, nx, ny),
                 DATA_TYPE POLYBENCH_2D(ey, NX, NY, nx, ny),
                 DATA_TYPE POLYBENCH_2D(hz, NX, NY, nx, ny)) {
  ...
  errcode =
    clEnqueueWriteBuffer(clCommandQue, fict_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * TMAX, _fict_, 0, NULL, NULL);
...
void cl_load_prog() {
  ...
  if (errcode != CL_SUCCESS)
    printf("Error in flushing command queue after kernel creation (%d)\n", errcode);
}
...
void cl_launch_kernel(int tmax, int nx, int ny) {
  ...
  for (t = 0; t < _PB_TMAX; t++) {
    ...
    errcode = clFinish(clCommandQue);
    if (errcode != CL_SUCCESS)
      printf("Error in flushing command queue after launching kernels (%d)\n", errcode);
  }

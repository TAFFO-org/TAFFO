...
#ifndef __TAFFO__
  fp = fopen("covariance.ptx", "r");
#else
  fp = fopen("covariance.taffo.ptx", "r");
#endif
  if (!fp) {
...
void cl_launch_kernel(int m, int n, DATA_TYPE* float_n) {
  ...
  // Execute the OpenCL kernel
  errcode = clEnqueueNDRangeKernel(
    clCommandQue, clKernel_reduce, 2, NULL, globalWorkSize_Kernel2, localWorkSize_Kernel2, 0, NULL, NULL);
  if (errcode != CL_SUCCESS)
  ...
  // Execute the OpenCL kernel
  errcode = clEnqueueNDRangeKernel(
    clCommandQue, clKernel_covar, 1, NULL, globalWorkSize_Kernel3, localWorkSize_Kernel3, 0, NULL, NULL);
  if (errcode != CL_SUCCESS)
...
int main(int argc, char* argv[]) {
  ...
  errcode = clEnqueueReadBuffer(
    clCommandQue, mean_mem_obj, CL_TRUE, 0, M * sizeof(DATA_TYPE), POLYBENCH_ARRAY(mean_gpu), 0, NULL, NULL);
  if (errcode != CL_SUCCESS)

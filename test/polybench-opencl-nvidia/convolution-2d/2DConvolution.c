...
void read_cl_file() {
  // Load the kernel source code into the array source_strf
#ifndef __TAFFO__
  fp = fopen("2DConvolution.ptx", "r");
#else
  fp = fopen("2DConvolution.taffo.ptx", "r");
#endif
  if (!fp) {
...
int main(int argc, char* argv[]) {
  ...
  errcode = clEnqueueReadBuffer(
    clCommandQue, b_mem_obj, CL_TRUE, 0, NI * NJ * sizeof(DATA_TYPE), POLYBENCH_ARRAY(B_outputFromGpu), 0, NULL, NULL);
  if (errcode != CL_SUCCESS)

...
#ifndef __TAFFO__
  fp = fopen("gemver.ptx", "r");
#else
  fp = fopen("gemver.taffo.ptx", "r");
#endif
  if (!fp) {
...
void cl_launch_kernel(int n, DATA_TYPE* alpha, DATA_TYPE* beta) {
  ...
  globalWorkSize[0] =
    (size_t) ceil(((float) N) / ((float) DIM_LOCAL_WORK_GROUP_KERNEL_1_X)) * DIM_LOCAL_WORK_GROUP_KERNEL_1_X;
  globalWorkSize[1] =
    (size_t) ceil(((float) N) / ((float) DIM_LOCAL_WORK_GROUP_KERNEL_1_Y)) * DIM_LOCAL_WORK_GROUP_KERNEL_1_Y;
  ...
  globalWorkSize[0] =
    (size_t) ceil(((float) N) / ((float) DIM_LOCAL_WORK_GROUP_KERNEL_2_X)) * DIM_LOCAL_WORK_GROUP_KERNEL_2_X;
  globalWorkSize[1] =
  ...
  globalWorkSize[0] =
    (size_t) ceil(((float) N) / ((float) DIM_LOCAL_WORK_GROUP_KERNEL_3_X)) * DIM_LOCAL_WORK_GROUP_KERNEL_3_X;
  globalWorkSize[1] =
...
int main(int argc, char* argv[]) {
  ...
  errcode = clEnqueueReadBuffer(
    clCommandQue, w_mem_obj, CL_TRUE, 0, N * sizeof(DATA_TYPE), POLYBENCH_ARRAY(w_outputFromGpu), 0, NULL, NULL);
  if (errcode != CL_SUCCESS)

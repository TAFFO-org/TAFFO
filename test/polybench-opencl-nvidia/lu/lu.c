...
void compareResults(int n,
                    DATA_TYPE POLYBENCH_2D(A_cpu, N, N, n, n),
  ...
  for (i = 0; i < n; i++) {
    for (j = 0; j < n; j++) {
      DATA_TYPE a = A_cpu[i][j];
      DATA_TYPE b = A_outputFromGpu[i][j];
...
void lu(int n, DATA_TYPE POLYBENCH_2D(A, N, N, n, n)) {
  ...
  for (k = 0; k < _PB_N; k++) {
    ...
    for (i = k + 1; i < _PB_N; i++) {
      for (j = k + 1; j < _PB_N; j++) {
        __attribute__((annotate("scalar()"))) DATA_TYPE tmp = A[i][k] * A[k][j];
        A[i][j] = A[i][j] - tmp;
      }
    }
  }

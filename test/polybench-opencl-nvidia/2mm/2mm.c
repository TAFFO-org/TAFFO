...
void cl_mem_init(DATA_TYPE POLYBENCH_2D(tmp, NI, NJ, ni, nj),
                 DATA_TYPE POLYBENCH_2D(A, NI, NK, ni, nk),
                 DATA_TYPE POLYBENCH_2D(B, NK, NJ, nk, nj),
                 DATA_TYPE POLYBENCH_2D(C, NL, NJ, nl, nj),
                 DATA_TYPE POLYBENCH_2D(D_outputFromGpu, NI, NL, ni, nl)) {
  ...
  errcode =
    clEnqueueWriteBuffer(clCommandQue, tmp_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * NI * NJ, tmp, 0, NULL, NULL);
  ...
  errcode = clEnqueueWriteBuffer(
    clCommandQue, dOutputFromGpu_mem_obj, CL_TRUE, 0, sizeof(DATA_TYPE) * NI * NL, D_outputFromGpu, 0, NULL, NULL);
  if (errcode != CL_SUCCESS)
...
void cl_launch_kernel(int ni, int nj, int nk, int nl, DATA_TYPE* alpha, DATA_TYPE* beta) {
  ...
  if (errcode != CL_SUCCESS)
    printf("Error in setting arguments\n");

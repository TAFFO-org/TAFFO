  ...
  for (r = 0; r < _PB_NR; r++) {
    for (q = 0; q < _PB_NQ; q++) {
      for (p = 0; p < _PB_NP; p++) {
        sum[r][q][p] = 0;
        for (s = 0; s < _PB_NP; s++) {
          float tmp = sum[r][q][p] + A[r][q][s] * C4[s][p];
          sum[r][q][p] = tmp;
        }
      }
      for (p = 0; p < _PB_NR; p++)
...
#ifndef __TAFFO__
  fp = fopen("doitgen.ptx", "r");
#else
  fp = fopen("doitgen.taffo.ptx", "r");
#endif
  if (!fp) {
...
void cl_mem_init(DATA_TYPE POLYBENCH_3D(A, NR, NQ, NP, nr, nq, np),
  ...
  errcode =
    clEnqueueWriteBuffer(clCommandQue, a_mem_obj, CL_TRUE, 0, NR * NQ * NP * sizeof(DATA_TYPE), A, 0, NULL, NULL);
  if (errcode != CL_SUCCESS)
  ...
  if (errcode != CL_SUCCESS)
    printf("Error in writing buffers\n");
  errcode =
    clEnqueueWriteBuffer(clCommandQue, c_mem_obj, CL_TRUE, 0, NR * NQ * NP * sizeof(DATA_TYPE), sum, 0, NULL, NULL);
  if (errcode != CL_SUCCESS)
    printf("Error in writing buffers\n");
}
...
int main(int argc, char* argv[]) {
  ...
  errcode = clEnqueueReadBuffer(
    clCommandQue, c_mem_obj, CL_TRUE, 0, NR * NQ * NP * sizeof(DATA_TYPE), sum_outputFromGpu, 0, NULL, NULL);
  if (errcode != CL_SUCCESS)

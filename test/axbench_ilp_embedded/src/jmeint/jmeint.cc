...
{                                                                              \
              ...
              if (sub1 >= 0.0f)                                                \
              {                                                                \
                float __attribute((annotate("scalar()"))) adj1 = sub1; \
              ...
              if (sub2 >= 0.0f)                                                \
              {                                                                \
                float __attribute((annotate("scalar()"))) adj2 = sub2; \
...
{                                              \
  ...
  float __attribute((annotate("errtarget('e') scalar()"))) e, __attribute((annotate("errtarget('d') scalar()"))) d, __attribute((annotate("errtarget('f') scalar()"))) f; \
  Ax=V1[i0]-V0[i0];                            \
  Ay=V1[i1]-V0[i1];                            \
...
{                                           \
  ...
  float __attribute((annotate("errtarget('d0*d1') scalar()")))d0d1, __attribute((annotate("errtarget('d0*d2') scalar()")))d0d2; \
...
static int tri_tri_intersect(float __attribute((annotate("scalar()"))) V0[3], float __attribute((annotate("scalar()"))) V1[3], float __attribute((annotate("scalar()"))) V2[3],
...
extern "C" int BENCH_MAIN(int argc, char* argv[])
{
	...
	while (i < n)
	{
		float __attribute((annotate("scalar(disabled range(-1,1))"))) a[18];
		for (int j=0; j<18; j++)
		  a[j] = jmeint_data[i].a[j];
		for(int j = 0 ; j < 18; j++)
		{
			xyz[i * 18 + j] = a[j];
		}

...
int main(int argc, char **argv)
{
  ...
  DATA_TYPE __attribute((annotate("scalar(range(260, 260) final)"))) float_n;
  POLYBENCH_2D_ARRAY_DECL(data, DATA_TYPE __attribute((annotate("scalar(range(-2, 2))"))), N, M, n, m);
  POLYBENCH_2D_ARRAY_DECL(corr, DATA_TYPE __attribute((annotate("target('corr') scalar(range(-2,2))"))), M, M, m, m);
  POLYBENCH_1D_ARRAY_DECL(mean, DATA_TYPE __attribute((annotate("scalar(range(-1,259))"))), M, m);
  POLYBENCH_1D_ARRAY_DECL(stddev, DATA_TYPE __attribute((annotate("scalar(range(-1,86) final)"))), M, m);

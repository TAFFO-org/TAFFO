...
Unless FOUR_WAY_MAX_WITH_REDUNDANCY is define'd to true, it uses the more concise version as per the comment at the end
of the file.

Programmed by Dave Wonnacott at Haverford College <davew@cs.haverford.edu>, with help from Allison Lake, Ting Zhou, and
Tian Jin, based on algorithm by Nussinov, described in Allison Lake's senior thesis.
...
For debugging/verifying we hit all the right points and have all updates before any pure reads:
   gcc -Dsize=250 -DCHECK_DEPENDENCES=1 -DVERBOSE=1 -DVERBOSE_OUT=stderr -Wall -Wno-unused-value -Wno-unknown-pragmas
Nussinov.c -o Nussinov && ./Nussinov 2> /tmp/Nussinov-log-250.txt ; sort /tmp/Nussinov-log-250.txt -o
/tmp/Nussinov-log-250.txt

-------------
Re-generate small or big matrix for comparison:
  gcc           -O3 -DPRINT_SIZE=5000 -Wall -Wno-unused-value -Wno-unknown-pragmas  Nussinov.c -o Nussinov-print && echo
250 | ./Nussinov-print > printed-matrix-250.out Or do the comparison gcc           -O3 -DPRINT_SIZE=5000 -Wall
-Wno-unused-value -Wno-unknown-pragmas  Nussinov.c -o Nussinov-print && echo 250 | ./Nussinov-print | diff
printed-matrix-250.out -

-------------
Re-generate big matrices for comparison:
  gcc           -O3 -Dsize=5000 -DPRINT_SIZE=5000 -Wall -Wno-unused-value -Wno-unknown-pragmas  Nussinov.c -o Nussinov
&& ./Nussinov > printed-matrix-5000.out

  gcc -DSEED=17 -O3 -Dsize=5000 -DPRINT_SIZE=5000 -Wall -Wno-unused-value -Wno-unknown-pragmas  Nussinov.c -o Nussinov
&& ./Nussinov > printed-matrix-5000-seed17.out
...
#define MAX_N_END(x, y) \
  (eassert(max_tmp_i == x && max_tmp_j == y), ((N_array[x][y]) = max_score(N_array[x][y], max_tmp)))
...
int getint(char* prompt) {
  ...
  int result;
  int i = 0;
  while (fprintf(stderr, "%s%s", prompt, terminate), result = scanf("%d", &i), result != 1 && result != EOF)
...
#if !defined size
  int size = getint(
    "Enter length of random mRNA sequence (2200 is average for human mRNA): "); // Average (human) mRNA length is 2200;
...
#pragma scop
  for (i = size - 1; i >= 0; i--) {
    for (j = i + 1; j < size; j++) {
      ...
      if (j - 1 >= 0 && i + 1 < size) {
        if (i < j - 1)
          MAX_N(i, j, N(i + 1, j - 1) + match(seq[i], seq[j])); /* don't allow adjacent elements to bond */
        else
          MAX_N(i, j, N(i + 1, j - 1));
      }
...
A: The original description of the Nussinov algorithm (#1 below) is equivalent to the simpler version (#2 below, also
Allie's thesis). (Note that all variants define N(i,j) for 1 <= i < j <= L; use N=0 when reading other elements.)
...
Case 2:	i < j-1
    here, N(i, j-1) is the same the max term's biggest k, i.e. k=j-1 gives N(i, j-1)+N(j-1+1, j) and the N(j,j) term is
0

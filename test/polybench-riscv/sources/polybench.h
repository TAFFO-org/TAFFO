...
float sqrtf_PB(float val)
{
  ...
  float diff;
  float min_tol = 0.001f;

  int i, flag;

  float constZeroVal = 0.0f;
  float constTwoVal = 2.0f;
  ...
  if (val == constZeroVal)
    ...
    for (i = 1; i < 20; i++) {
      if (!flag) {
        dx = (val - (x * x)) / (constTwoVal * x);
        x = x + dx;
        diff = val - (x * x);
        if ((diff >= constZeroVal? diff : -diff) <= min_tol)
          flag = 1;
      }
    }
  }
...
#  define POLYBENCH_DCE_ONLY_CODE    if (argc > 42 && ! strcmp(argv[0], "scalar"))
# else

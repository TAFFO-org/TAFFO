...
def set_up(path):
    bench_name = path.name + ".cpp"
    with open("stub.h", "w") as file:
        file.write(f"#include \"{path}/{bench_name}\"")
...
def compilefloat(path: Path):
    ...
    print("Compiling: {}\t".format(bench_exec), end="")
    flush()
    s = subprocess.run(f"make clean; make; mv main.bin {path}/{bench_exec}", shell=True, stdout=pipe_out, stderr=pipe_out)
...
def run(path :Path):
    ...
    if not ((path / "{}-taffo".format(bench_name)).exists() and (path / "{}-float".format(bench_name)).exists()):
        ...
        s1 = subprocess.run("cp {0}/{1}-float main.bin;  st-flash  --reset write \"main.bin\" 0x8000000 ; sleep 1; dump_serial /dev/ttyACM0 19200 \"Values End\" 10 | tee float-res; cp float-res {0}/float-res".format(path.as_posix(), bench_name,), shell=True)
        s = subprocess.run("cp {0}/{1}-taffo main.bin;  st-flash  --reset write \"main.bin\" 0x8000000 ; sleep 1; dump_serial /dev/ttyACM0 19200 \"Values End\" 10  | tee taffo-res; cp taffo-res {0}/taffo-res".format(path.as_posix(), bench_name,), shell=True)
...
if __name__ == '__main__':
    ...
    M = args.M
    common_flags = f"-O3 -fno-vectorize -fno-slp-vectorize"
    if args.only is None:
        only = [x for x in Path(".").glob("*/") if x.is_dir() and "data" not in x.name and "config" not in x.name]
    else:

...
float __attribute__((annotate("scalar(range(-176586304, 7394583.5) type(64 35) final)"))) ex0(float lat1, float lat2, float lon1, float lon2)
{
  ...
  float __attribute__((annotate("scalar(range(-100, 100) type(64 35) final)"))) inner_atan = ((c_lat2 * s_dLon) / ((c_lat1 * s_lat2) - ((s_lat1 * c_lat2) * c_dLon)));
...
int internal_main()
{
  ...
  float __attribute__((annotate("scalar(range(-1, 1) type(64 60) final) ")))
  lat1[len];

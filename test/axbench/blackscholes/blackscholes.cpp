...
fptype CNDF(fptype __attribute((annotate("scalar()"))) InputX) {
  ...
  fptype __attribute((annotate("scalar(range(30,12412))"))) InputXX;
...
int bs_thread(void* tid_ptr) {
  ...
  for (j = 0; j < NUM_RUNS; j++) {
    for (i = start; i < end; i++) {
       ...
       */
      fptype __attribute((annotate("scalar()"))) N1, __attribute((annotate("scalar()"))) N2;
      ...
      */
      price_orig =
        BlkSchlsEqEuroNoDiv(sptprice[i], strike[i], rate[i], volatility[i], otime[i], otype[i], timet, &N1, &N2);
      /*

...
int main(int argc, const char* argv[]) {
  ...
  for (int i = 0; i < n * 2 * 2; i += 2 * 2) {
    float __attribute((annotate("scalar(" ANNOTATION_RANGE " error(1e-8) disabled)"))) theta1,
      __attribute((annotate("scalar(" ANNOTATION_RANGE " error(1e-8) disabled)"))) theta2;

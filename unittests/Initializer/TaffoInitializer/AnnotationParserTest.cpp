...
TEST_F(AnnotationParserTest, ScalarRepeatedAttributes) {
  ...
  annstr = llvm::StringRef(
    "scalar(range(0, 10) type(1 2) error(3.1415) range(15, 20) type(unsigned 3 4) error(1.6180) disabled final)");
  res = parser.parseAnnotationString(annstr);
  error = parser.lastError().str();
  EXPECT_EQ(NO_ERR, error);
  ASSERT_TRUE(res);
...
TEST_F(AnnotationParserTest, ScalarDup) {
  ...
  annstr = llvm::StringRef(
    "scalar(range(0, 10) type(1 2) error(3.1415)) scalar(range(15, 20) type(unsigned 3 4) error(1.6180))");
  res = parser.parseAnnotationString(annstr);
  error = parser.lastError().str();
...
TEST_F(AnnotationParserTest, StructMultipleStructs) {
  ...
  annstr =
    llvm::StringRef("struct[struct[scalar(error(3.1415))], struct[scalar(error(1.6180)), scalar(error(2.7183))]]");
  res = parser.parseAnnotationString(annstr);
  error = parser.lastError().str();
  EXPECT_EQ(NO_ERR, error);
  ASSERT_TRUE(res);
...
TEST_F(AnnotationParserTest, StringAllLiterals) {
  ...
  annstr = llvm::StringRef("target('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!#$%&()*+,-./"
                           ":;<=>?[]^_`{|}~\"\\') struct[void]");
  res = parser.parseAnnotationString(annstr);
  error = parser.lastError().str();
  EXPECT_EQ(NO_ERR, error);
